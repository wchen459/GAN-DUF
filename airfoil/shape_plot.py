...
def plot_samples(Z, X, scale=0.8, points_per_axis=None, scatter=True, symm_axis=None, annotate=False, z0=None, x0=None, fname=None, **kwargs):
...
#    plt.ylabel('c2')
    ...
    if z0 is not None and x0 is not None:
        ...
        plot_shape(x0, z0[0], 0.3*z0[1], ax, scale, scatter, symm_axis, **kwargs)
        plt.title('({:.2f},{:.2f})'.format(z0[0], z0[1]))
        plt.xticks([])
        plt.yticks([])
        plt.axis('off')
        plt.axis('equal')
...
def plot_synthesized(Z, gen_func, d=2, scale=.8, points_per_axis=None, scatter=True, symm_axis=None, fname=None, **kwargs):
    ...
    if type(X).__name__ == 'tuple':
        X1, z0, x0 = X
        plot_samples(Z, X1, scale, points_per_axis, scatter, symm_axis, z0=z0, x0=x0, fname=fname, **kwargs)
    else:
